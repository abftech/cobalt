...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        batch = BatchStatus.objects.create(command="update_membership_status")
        summary_lines = []
        ...
        try:
            ...
            summary_lines.append(
                f"Due-date phase: {self.due_processed} lapsed, "
                f"{self.due_errored} errored, {self.due_skipped} no action required"
            )
            self.processed = 0
            self.errored = 0
            self.skipped = 0
            ...
            summary_lines.append(
                f"End-date phase: {self.processed} transitioned, "
                f"{self.errored} errored, {self.skipped} no action required"
            )
            summary_lines.append(
                f"Total: {self.processed + self.due_processed} processed, "
            ...
            logger.exception(
                "Membership status update failed with an unhandled exception"
            )
