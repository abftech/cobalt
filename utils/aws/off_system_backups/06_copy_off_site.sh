...
echo "Running: aws s3 --profile backblaze cp \"$LATEST_DB\" s3://abf-offsite-backups"
...
echo "Running: aws s3 --profile backblaze cp \"$LATEST_FILE_SYSTEM\" s3://abf-offsite-backups"
if ! aws s3 --profile backblaze cp "$LATEST_FILE_SYSTEM" s3://abf-offsite-backups
then
  ./notify.sh error "Error copying file system backup file to BackBlaze"
  exit 1
fi
