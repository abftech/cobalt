...
def _helper_thread_function(index, member, count):
...
class PlayerMultiPaymentTests:
    ...
    def account_update(self):
        ...
        for index in range(thread_count):
            this_thread = Thread(
                target=_helper_thread_function, args=[index, player, repeats]
            )
        ...
        # Check results
        results = MemberTransaction.objects.filter(member=player).order_by("pk")
        ...
        self.manager.save_results(
            ...
            test_description=f"Run multiple thread to update user {player}. Should be processed in order.",
